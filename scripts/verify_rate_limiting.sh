...
# Test 1: Unit tests
echo "1️⃣  Executando testes unitários..."
if python -m pytest tests/test_rate_limiting.py -v --tb=short; then
    echo -e "${GREEN}✅ Testes unitários: OK (11/11)${NC}"
else
...
echo "2️⃣  Executando testes de integração..."
if python -m pytest tests/test_rate_limiting_integration.py -v --tb=short; then
    echo -e "${GREEN}✅ Testes de integração: OK (7/7)${NC}"
else
...
# Test 3: Import check
echo "3️⃣  Verificando imports dos middlewares..."
if python -c "from app.middleware import get_global_rate_limiter, get_quota_tracker; print('✓ Imports OK')"; then
    echo -e "${GREEN}✅ Middlewares importam corretamente${NC}"
else
    echo -e "${RED}❌ Erro ao importar middlewares${NC}"
    exit 1
fi
echo ""

# Test 4: Singleton pattern
echo "4️⃣  Verificando padrão singleton..."
python << 'PYEOF'
...
PYEOF
echo -e "${GREEN}✅ Padrão singleton: OK${NC}"
echo ""

# Test 5: Data persistence
echo "5️⃣  Verificando persistência de dados..."
if [ -f "./data/quota_usage.json" ]; then
    echo -e "${GREEN}✅ Arquivo de quota existe: ./data/quota_usage.json${NC}"
    echo -e "${YELLOW}Conteúdo:${NC}"
    cat ./data/quota_usage.json | jq . 2>/dev/null || cat ./data/quota_usage.json
else
    echo -e "${YELLOW}⚠️  Arquivo de quota ainda não criado (será criado no primeiro uso)${NC}"
fi
echo ""

# Test 6: File structure
echo "6️⃣  Verificando estrutura de arquivos..."
REQUIRED_FILES=(
...
echo "7️⃣  Verificando arquivos modificados..."
MODIFIED_FILES=(
    "app/main.py"
...
# Summary
echo "═══════════════════════════════════════════════════════════════"
echo -e "${GREEN}  ✅ VERIFICAÇÃO COMPLETA - SISTEMA FUNCIONANDO${NC}"
echo "═══════════════════════════════════════════════════════════════"
echo ""
